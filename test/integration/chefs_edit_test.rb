...
class ChefsEditTest < ActionDispatch::IntegrationTest
  def setup 
    ...
    @admin_user = Chef.create!(chefname: "Kenneth3", email: "kenneth3@example.com",
                        password: "password3", password_confirmation: "password3", admin: true)
  end
  ...
  test "accept edit attemt by admin user" do
    sign_in_as(@admin_user, "password3")
    get edit_chef_path(@chef)
    assert_template 'chefs/edit'
  ...
  test "redirect edit attemt by another non-admin user" do
    ...
    assert_not flash.empty?
    @chef.reload
    assert_match "Kenneth", @chef.chefname
    assert_match "kenneth@example.com", @chef.email
  end
end
